...
def init_db():
    ...
    # Check if the store is empty. If it is, seed it with starter items automatically.
    cursor.execute("SELECT COUNT(*) FROM products")
    if cursor.fetchone()[0] == 0:
        cursor.execute("INSERT INTO products (name, price, description, image) VALUES (?, ?, ?, ?)",
                       ("Cool Sneakers", "$50.00", "Super comfortable running shoes.", "https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=500"))
        cursor.execute("INSERT INTO products (name, price, description, image) VALUES (?, ?, ?, ?)",
                       ("Vintage Watch", "$120.00", "A classic timepiece that never goes out of style.", "https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=500"))
        cursor.execute("INSERT INTO products (name, price, description, image) VALUES (?, ?, ?, ?)",
                       ("Wireless Headphones", "$85.00", "Crystal clear sound with deep bass.", "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=500"))
        conn.commit()
